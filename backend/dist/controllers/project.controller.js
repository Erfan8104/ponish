...
exports.getFreelancerContracts = exports.acceptProposal = exports.getProjectProposals = exports.getMyProjects = exports.submitProposal = exports.deleteProject = exports.updateProject = exports.getProjectById = exports.getProjects = exports.createProject = void 0;
const prisma_1 = require("../lib/prisma");
const client_1 = require("@prisma/client");
const project_validator_1 = require("../validators/project.validator");
const project_validator_2 = require("../validators/project.validator");
/**
 * =========================
 * Helper: normalize multipart body
 * =========================
 */
const preprocessMultipartData = (body) => {
    ...
    if (processed.calculatedArea !== undefined &&
...
const createProject = async (req, res) => {
    try {
        ...
        const validation = project_validator_1.createProjectSchema.safeParse(processedBody);
        ...
        let categoryId = null;
        if (data.category) {
            const foundCategory = await prisma_1.prisma.category.findUnique({
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            const newProject = await tx.project.create({
                data: {
                    ...
                    minBudget: data.minBudget ? new client_1.Prisma.Decimal(data.minBudget) : null,
                    maxBudget: data.maxBudget ? new client_1.Prisma.Decimal(data.maxBudget) : null,
...
const getProjects = async (req, res) => {
    try {
        const { category, search, province, city, budgetType, page = 1, limit = 10, } = req.query;
        ...
        if (budgetType)
            whereClause.budgetType = budgetType;
        if (search) {
            whereClause.OR = [
                {
                    title: {
...
const getProjectById = async (req, res) => {
    try {
        ...
        const proposalCount = await prisma_1.prisma.proposal.count({
            where: {
                projectId: Number(id),
            },
        });
...
const updateProject = async (req, res) => {
    try {
        ...
        const validation = project_validator_2.updateProjectSchema.safeParse(processedBody);
        if (!validation.success) {
            return res.status(400).json({
                success: false,
                errors: validation.error.issues,
            ...
            const updatedProject = await tx.project.update({
                ...
                data: {
                    ...
                    minBudget: validation.data.minBudget
                        ? new client_1.Prisma.Decimal(validation.data.minBudget)
                        : undefined,
                    maxBudget: validation.data.maxBudget
                        ? new client_1.Prisma.Decimal(validation.data.maxBudget)
                        : undefined,
                },
            });
            /**
...
exports.getFreelancerContracts = getFreelancerContracts;
